...
CREATE OR REPLACE function batacharyaHistFunc(nameInput IN varchar2, seuil IN double precision ) RETURN BATRESULT
...
CREATE OR REPLACE TYPE trio IS VARRAY(3) OF double precision;
/
...
CREATE OR REPLACE function SigLoopFunc( nameInput IN varchar2, color IN integer, texture IN integer, shape IN integer, locationn IN integer, seuil IN integer) return SIGRESULT
IS
...
CREATE OR REPLACE function NBPIXELSLoopFunc( nameInput IN varchar2, seuil IN integer) return SIGRESULT
IS
  ...
  BEGIN
    ...
    select NBPIXELS into nbPixels from multimedia where nom like nameInput; 
    open c1;
